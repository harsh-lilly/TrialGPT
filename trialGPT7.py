...
def get_matching_prompt(inc_exc, trial_info, patient):
	...
	prompt = f"You are a helpful assistant for clinical trial recruitment. Your task is to compare a given patient note and the {inc_exc} criteria of a clinical trial to determine the patient's eligibility at the criterion level. You've two tasks:\n"
	prompt += "Task 1: The factors that allow someone to participate in a clinical study are called inclusion criteria. The criterias that restrict/disqualify a patient from particpating are called exclusion criterias.\n"
	...
	prompt += "Important: To determine if the patient matches a given inclusion or exclusion criteria your conclusion needs to be derived from the patient information and not assumption.\n"
	...
	prompt += "Task 2: To compute score relevance score (R) between the patient and the clinical trial.\n"
	prompt += "First, briefly explain if and how the patient is relevant, then compute a score between 0 to 100. Where 0 represents the patient is totally irrelevant and 100 represents exaclty relevant.\n"
	prompt += "Don't output any extra information but only output a JSON dict formatted as: dict{ 'inclusion_criteria_match': [1,3,6,...etc], 'exclusion_criteria_match': [1,3,6,...etc], 'relevance_explanation': str(explanation), 'relevance_score_R': int(score)} \n\n"
